...
/**
     ...
     * ```
     */
function findAllIndexOf(str, character) {
    let r = [];
...
function addZeroesToNumber(num, amount) {
    let t = num.toString();
    if (t.length === amount) {
        return `${num}`;
    }
...
export function descobreCPFs(cpf, completo) {
    let cpfs = [];
    let n = findAllIndexOf(cpf, 'x').length;
    for (let i = 0; i < getNumeral(n); i++) {
        let _cpf = replaceFromCPF(cpf, i);
        if (validateCPF(_cpf)) {
            cpfs.push(completo ? { cpf: _cpf, estado: getEstado(_cpf) } :
                _cpf);
        }
    }
    return cpfs;
